...
class BVFSCallback extends BVCallbackBase {
	...
	function getFilesContent($files, $withContent = true) {
		...
		foreach ($files as $file) {
			...
			if ((CWMWPFileSystem::getInstance()->isDir($absfile) === true) && !is_link($absfile)) {
				$fdata['is_dir'] = true;
			} else {
				if (isset($fdata["error"]) && $fdata["error"] === "NOT_READABLE") {
					...
					if ($withContent === true) {
						$content = CWMWPFileSystem::getInstance()->getContents($absfile);
						if ($content !== false) {
	...
	function uploadFiles($files, $offset = 0, $limit = 0, $bsize = 102400) {
		$result = array();
		foreach ($files as $file) {
			if (CWMWPFileSystem::getInstance()->isReadable(ABSPATH.$file) === false) {
				$result["missingfiles"][] = $file;
				continue;
			}
