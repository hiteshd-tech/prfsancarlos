...
class BVAccountCallback extends BVCallbackBase {
	...
	function updateInfo($args) {
		...
		if (array_key_exists('update_info', $args)) {
			...
			$result['update_info'] = array(
				"status" => CWMAccount::exists($this->settings, $args['update_info']['pubkey'])
			);
		}
	
		if (array_key_exists('update_api_key', $args)) {
			CWMAccount::updateApiPublicKey($this->settings, $args['update_api_key']['pubkey']);
			$result['update_api_key'] = array(
				"status" => $this->settings->getOption(CWMAccount::$api_public_key)
			);
		}
	...
	function process($request) {
		...
		case "addacc":
			CWMAccount::addAccount($settings, $params['public'], $params['secret']);
			$resp = array("status" => CWMAccount::exists($settings, $params['public']));
			break;
		case "rmacc":
			$resp = array("status" => CWMAccount::remove($settings, $params['public']));
			break;
		case "updt":
			$account->updateInfo($params);
			$resp = array("status" => CWMAccount::exists($settings, $params['pubkey']));
			break;
		case "updtapikey":
			CWMAccount::updateApiPublicKey($settings, $params['pubkey']);
			$resp = array("status" => $settings->getOption(CWMAccount::$api_public_key));
			break;
		case "rmbvscrt":
			$resp = array("status" => CWMRecover::deleteDefaultSecret($settings));
			break;
		case "rmbvkeys":
		...
		case "rmoldbvacc":
			...
			break;
		case "fetch":
			$accounts = CWMAccount::allAccounts($settings);
