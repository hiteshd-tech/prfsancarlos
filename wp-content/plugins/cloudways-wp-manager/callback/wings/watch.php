...
class BVWatchCallback extends BVCallbackBase {
	...
	public function getFWPrependLog($params) {
		...
		if (CWMWPFileSystem::getInstance()->exists($fname) === true) {

			$result['exists'] = true;
			$tmpfname = $fname."tmp";

			if (CWMWPFileSystem::getInstance()->move($fname, $tmpfname, true) === false) {
				$result = array('status' => 'Error', 'message' => 'UNABLE_TO_RENAME_LOGFILE');

			} else {

				if (CWMWPFileSystem::getInstance()->exists($tmpfname) === true) {

					$fsize = CWMWPFileSystem::getInstance()->size($tmpfname);
					$result["size"] = $fsize;

					if ($fsize <= $limit) {
						$result['content'] = CWMWPFileSystem::getInstance()->getContents($tmpfname);
					} else {
	...
	public function process($request) {
		...
		case "getdata":
			...
			if (array_key_exists('lp', $params)) {
				...
				$lp_params = $params['lp'];
				if (!isset($lp_params['bv_check_table']) || $db->isTablePresent($db->getBVTable(CWMProtectLP_V629::TABLE_NAME))) {
					$limit = intval($lp_params['limit']);
					$filter = $lp_params['filter'];
					$offset = isset($lp_params['offset']) ? intval($lp_params['offset']) : 0;
					$table = $db->getBVTable(CWMProtectLP_V629::TABLE_NAME);
					$resp["lplogs"] = $this->getOffsetResetInfo($lp_params, $offset, $table);
					if (empty($resp["lplogs"])) {
						$db->deleteBVTableContent(CWMProtectLP_V629::TABLE_NAME, $lp_params['rmfilter']);
			...
			if (array_key_exists('fw', $params)) {
				...
				$fw_params = $params['fw'];
				if (!isset($fw_params['bv_check_table']) || $db->isTablePresent($db->getBVTable(CWMProtectFW_V629::TABLE_NAME))) {
					$limit = intval($fw_params['limit']);
					$filter = $fw_params['filter'];
					$offset = isset($fw_params['offset']) ? intval($fw_params['offset']) : 0;
					$table = $db->getBVTable(CWMProtectFW_V629::TABLE_NAME);
					$resp["fwlogs"] = $this->getOffsetResetInfo($fw_params, $offset, $table);
					if (empty($resp["fwlogs"])){
						$db->deleteBVTableContent(CWMProtectFW_V629::TABLE_NAME, $fw_params['rmfilter']);
			...
			if (array_key_exists('php_error_monitoring', $params)) {
				...
				$table = $db->getBVTable(CWMWPPHPErrorMonitoring::ERROR_TABLE);
				if (!isset($php_error_monit_params['bv_check_table']) || $db->isTablePresent($table)) {
					...
					if (empty($resp["php_error_monitoring"])) {
						$db->deleteBVTableContent(CWMWPPHPErrorMonitoring::ERROR_TABLE, $php_error_monit_params['rmfilter']);
