...
class BVFSWriteCallback extends BVCallbackBase {
	...
	public function removeDirs($dirs) {
		...
		foreach ($dirs as $dir) {
			...
			if ((CWMWPFileSystem::getInstance()->isDir($dir) === true) && !is_link($dir)) {
				if ($this->isEmptyDir($dir)) {
					$dir_result['status'] = CWMWPFileSystem::getInstance()->rmdir($dir);
					if ($dir_result['status'] === false) {
						$dir_result['error'] = "RMDIR_FAILED";
						$fs_error = CWMWPFileSystem::getInstance()->checkForErrors();
						if (isset($fs_error)) {
	...
	public function doChmod($path_infos) {
		...
		foreach ($path_infos as $path => $mode) {
			...
			if (CWMWPFileSystem::getInstance()->exists($path) === true) {
				$path_result['status'] = CWMWPFileSystem::getInstance()->chmod($path, $mode);
	...
	public function renameFiles($path_infos) {
		...
		foreach ($path_infos as $oldpath => $newpath) {
			...
			if (CWMWPFileSystem::getInstance()->exists($oldpath)) {
				$action_result['status'] = CWMWPFileSystem::getInstance()->move($oldpath, $newpath, true);
	...
	public function writeContentToFile($content, $ofile) {
		...
		if (CWMWPFileSystem::getInstance()->putContents($ofile, $content) === false) {
			$result['error'] = 'UNABLE_TO_WRITE_TO_TMP_OFILE';
			$fs_error = CWMWPFileSystem::getInstance()->checkForErrors();
			if (isset($fs_error)) {
	...
	public function moveUploadedFile($ofile) {
		...
		if (isset($_FILES['myfile'])) {
			...
			if ($is_upload_ok) {
				...
				if (CWMWPFileSystem::getInstance()->move($tmp_name, $ofile, true) === false) {
