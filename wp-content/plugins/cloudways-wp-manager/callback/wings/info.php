...
class BVInfoCallback extends BVCallbackBase {
	...
	public function __construct($callback_handler) {
		...
		$this->bvinfo = new CWMInfo($this->settings);
		$this->bvapi = new CWMWPAPI($this->settings);
	}
	...
	public function getSystemInfo() {
		$sys_info = array(
			'host' => CWMHelper::getRawParam('SERVER', 'HTTP_HOST'),
			'phpversion' => phpversion(),
			'AF_INET6' => defined('AF_INET6')
		);
		if (array_key_exists('SERVER_ADDR', $_SERVER)) {
			$sys_info['serverip'] = CWMHelper::getRawParam('SERVER', 'SERVER_ADDR');
		}
	...
	public function getWpInfo() {
		...
		$wp_info = array(
			...
			'abspath' => ABSPATH,
			'bvpluginpath' => defined('CWMBASEPATH') ? CWMBASEPATH : null,
	...
	public function getHostInfo() {
		...
		if (isset($_SERVER['SERVER_ADDR']) && function_exists('gethostbyaddr')) {
			$host_info['HOST_FROM_IP'] = gethostbyaddr(CWMHelper::getRawParam('SERVER', 'SERVER_ADDR'));
		}
	...
	public function serverConfig() {
		return array(
			'software' => CWMHelper::getRawParam('SERVER', 'SERVER_SOFTWARE'),
