	...
	class BVCallbackRequest {
		...
		public function __construct($account, $in_params, $settings) {
			...
			$this->pubkey_name = !empty($in_params['pubkeyname']) ?
					CWMAccount::sanitizeKey($in_params['pubkeyname']) : 'm_public';
			$this->bvprmsmac = !empty($in_params['bvprmsmac']) ? CWMAccount::sanitizeKey($in_params['bvprmsmac']) : "";
		...
		public function verify($data, $sig, $sighshalgo) {
			...
			$public_key_str = CWMWPFileSystem::getInstance()->getContents($key_file);
		...
		public function corruptedParamsResp() {
			$bvinfo = new CWMInfo($this->settings);
		...
		public function authFailedResp() {
			$api_public_key = CWMAccount::getApiPublicKey($this->settings);
			$default_secret = CWMRecover::getDefaultSecret($this->settings);
			$default_account_pubkey = CWMAccount::getDefaultPublicKey();
			$bvinfo = new CWMInfo($this->settings);
			$resp = array(
				"request_info" => $this->info(),
				"bvinfo" => $bvinfo->info(),
				...
				"def_key_status" => CWMRecover::getSecretStatus($this->settings),
