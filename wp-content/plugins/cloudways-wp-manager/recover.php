...
if (!class_exists('CWMRecover')) :
	class CWMRecover {
		...
		public static function defaultSecret($settings) {
			...
			if (empty($secret)) {
				$secret = CWMRecover::refreshDefaultSecret($settings);
			}
			return $secret;
		}
		...
		public static function refreshDefaultSecret($settings) {
			$key_details = array();
			$key_details["key"] = CWMAccount::randString(32);
		...
		public static function find($settings, $pubkey) {
			...
			$account = new CWMAccount($settings, $pubkey, $secret);
			return $account;
		}
	}
endif;