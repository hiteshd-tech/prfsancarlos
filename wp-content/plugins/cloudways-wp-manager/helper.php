...
if (!class_exists('CWMHelper')) :
	class CWMHelper {
		...
		public static function fileRemovePattern($fname, $pattern, $is_regex = false) {
			...
			if (!CWMWPFileSystem::getInstance()->exists($fname)) {
				return;
			}

			$content = CWMWPFileSystem::getInstance()->getContents($fname);
			if ($content !== false) {
				if ($is_regex !== false) {
				...
				if ($content !== $modified_content) {
					CWMWPFileSystem::getInstance()->putContents($fname, $modified_content,
							CWMWPFileSystem::getInstance()->getchmodOctal($fname));
				}
			}
		}
