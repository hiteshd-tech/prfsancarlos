...
class CWMWPSiteInfo {
	...
	public function basic(&$info) {
		...
		if (array_key_exists('SERVER_ADDR', $_SERVER)) {
			$info['serverip'] = CWMHelper::getRawParam('SERVER', 'SERVER_ADDR');
		}
	...
	public function serversig($full = false) {
		$sig_param = ABSPATH;
		$server_addr = CWMHelper::getRawParam('SERVER', 'SERVER_ADDR');
	...
	public function dbsig($full = false) {
		if (defined('DB_USER') && defined('DB_NAME') &&
			...
			$sig = "bvnone".CWMAccount::randString(34);
		}
		if ($full)
			return $sig;
		else
