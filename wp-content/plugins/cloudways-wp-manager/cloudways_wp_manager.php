<?php
/*
Plugin Name: Cloudways WP Manager
Plugin URI: https://www.cloudways.com
Description: The easiest way to manage your site on Cloudways
Author: Cloudways
...
/*  Copyright 2017  Cloudways WP Manager  (email : support@blogvault.net)
...
$bvsettings = new CWMWPSettings();
$bvsiteinfo = new CWMWPSiteInfo();
$bvdb = new CWMWPDb();


$bvapi = new CWMWPAPI($bvsettings);
$bvinfo = new CWMInfo($bvsettings);
$wp_action = new CWMWPAction($bvsettings, $bvsiteinfo, $bvapi);

register_uninstall_hook(__FILE__, array('CWMWPAction', 'uninstall'));
register_activation_hook(__FILE__, array($wp_action, 'activate'));
register_deactivation_hook(__FILE__, array($wp_action, 'deactivate'));


add_action('wp_footer', array($wp_action, 'footerHandler'), 100);
add_action('cwm_clear_bv_services_config', array($wp_action, 'clear_bv_services_config'));
...
if (defined('WP_CLI') && WP_CLI) {
		...
		$wp_cli = new CWMWPCli($bvsettings, $bvinfo, $bvsiteinfo, $bvapi);
		WP_CLI::add_command("cloudways_wp_manager", $wp_cli);
}

if (is_admin()) {
	require_once dirname( __FILE__ ) . '/wp_admin.php';
	$wpadmin = new CWMWPAdmin($bvsettings, $bvsiteinfo);
...
if (CWMHelper::getRawParam('REQUEST', 'bvplugname') == "cloudways_wp_manager") {
	require_once dirname( __FILE__ ) . '/callback/base.php';
	require_once dirname( __FILE__ ) . '/callback/response.php';
	require_once dirname( __FILE__ ) . '/callback/request.php';
	require_once dirname( __FILE__ ) . '/recover.php';

	$pubkey = CWMHelper::getRawParam('REQUEST', 'pubkey');
	$pubkey = isset($pubkey) ? CWMAccount::sanitizeKey($pubkey) : '';
	$rcvracc = CWMHelper::getRawParam('REQUEST', 'rcvracc');

	if (isset($rcvracc)) {
		$account = CWMRecover::find($bvsettings, $pubkey);
	} else {
		$account = CWMAccount::find($bvsettings, $pubkey);
	}
	...
	if ($request->authenticate() === 1) {
		$bv_frm_tstng = CWMHelper::getRawParam('REQUEST', 'bv_frm_tstng');
